...
import { api } from "../../auth/api.js"; // Importe sua instância 'api' configurada (ex: axios)
...
export default function Configuracoes() {
  ...
  const twoFA = settings.security?.twoFactor || {
    enabled: false,
    secretSet: false,
  };
  ...
  const handleSaveEmail = () => {
    ...
    if (!ok)
      return setMsg((m) => ({ ...m, conta: "Informe um e-mail válido." }));
  ...
  const handleFileChange = async (event) => {
    ...
    if (!file.type?.startsWith("image/"))
      return setMsg((m) => ({ ...m, foto: "Tipo de arquivo não suportado." }));
    ...
    try {
      ...
      setMsg((m) => ({
        ...m,
        conta: err.response?.data?.error || err.message || "Falha no upload.",
      }));
    } finally {
      setUploading(false);
    }
  ...
  const handleLinkPhoto = async () => {
    if (!linkUrl)
      return setMsg((m) => ({ ...m, conta: "Insira uma URL válida." }));
    setUploading(true);
    try {
      // Usando a rota do profileController
      ...
      setMsg((m) => ({
        ...m,
        conta:
          err.response?.data?.error || err.message || "Falha ao vincular URL.",
      }));
    } finally {
      setUploading(false);
    }
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      ...
      setMsg((m) => ({
        ...m,
        conta:
          err.response?.data?.error || err.message || "Falha ao remover foto.",
      }));
    } finally {
      setUploading(false);
    }
  ...
  const handleChangePassword = () => {
    ...
    if (!pwCurrent || !pwNext || !pwConfirm)
      return setMsg((m) => ({ ...m, senha: "Preencha todos os campos." }));
    if (pwNext.length < 6)
      return setMsg((m) => ({
        ...m,
        senha: "Nova senha deve ter no mínimo 6 caracteres.",
      }));
    if (pwNext !== pwConfirm)
      return setMsg((m) => ({ ...m, senha: "A confirmação não confere." }));
    ...
    setMsg((m) => ({
      ...m,
      senha: res.ok ? "Senha alterada (simulação local)." : res.error,
    }));
    if (res.ok) {
      setPwCurrent("");
      setPwNext("");
  ...
  const handle2FAToggle = (checked) => {
    ...
    setMsg((m) => ({
      ...m,
      seguranca: checked ? "2FA habilitado (simulação)." : "2FA desabilitado.",
    }));
    clearMsg("seguranca");
  };
    ...
    <div className="cfg-container">
      ...
      <nav className="cfg-tabs">
        <button
          ...
          role="tab"
          aria-selected={secaoAtiva === "conta"}
        >
          <i className="fa-solid fa-user-pen"></i> Conta
        </button>
        <button
          className={`cfg-tab-button ${
            secaoAtiva === "seguranca" ? "active" : ""
          }`}
          onClick={() => setSecaoAtiva("seguranca")}
          role="tab"
          aria-selected={secaoAtiva === "seguranca"}
        >
          <i className="fa-solid fa-shield-halved"></i> Segurança
        </button>
        <button
          className={`cfg-tab-button ${
            secaoAtiva === "aparencia" ? "active" : ""
          }`}
          onClick={() => setSecaoAtiva("aparencia")}
          role="tab"
          aria-selected={secaoAtiva === "aparencia"}
        >
          <i className="fa-solid fa-palette"></i> Aparência
        </button>
        <button
          className={`cfg-tab-button ${
            secaoAtiva === "presenca" ? "active" : ""
          }`}
          onClick={() => setSecaoAtiva("presenca")}
          role="tab"
          aria-selected={secaoAtiva === "presenca"}
        >
        ...
        <section className="cfg-card" role="tabpanel">
          ...
          <div className="cfg-row-vertical">
            <div className="cfg-label">
              ...
              <small className="muted">
                Esta foto aparece no seu perfil e nos cards de grupo.
              </small>
            </div>
          ...
          <div className="cfg-row-vertical">
            <div className="cfg-label">
              ...
              <small className="muted">
                Use uma imagem de um site (ex: Gravatar, GitHub).
              </small>
            </div>
          ...
          {msg.conta && (
            <p
              className={`cfg-msg ${
                msg.conta.includes("sucesso") ? "success" : "error"
              }`}
            >
              {msg.conta}
            </p>
      ...
      {secaoAtiva === "seguranca" && (
        <section className="cfg-card" role="tabpanel">
          ...
          {msg.senha && (
            <p
              className={`cfg-msg ${
                msg.senha.includes("sucesso") ? "success" : "error"
              }`}
            >
              {msg.senha}
            </p>
          ...
          <div className="cfg-row">
            ...
            <div className="cfg-control cfg-inline">
              ...
              <span>
                {twoFA.enabled ? "2FA Habilitado" : "2FA Desabilitado"}
              </span>
            </div>
          ...
          {msg.seguranca && (
            <p
              className={`cfg-msg ${
                msg.seguranca.includes("sucesso") ? "success" : "error"
              }`}
            >
              {msg.seguranca}
            </p>
        ...
        <section className="cfg-card" role="tabpanel">
          ...
          <div className="cfg-row">
            <div className="cfg-label">
              ...
              <small className="muted">
                Afeta todas as páginas, exceto o Painel Principal.
              </small>
            </div>
            <div className="cfg-control cfg-inline">
              <label>
                <input
                  ...
                  name="theme"
                  value="claro"
                  checked={themeMode === "claro"}
                  onChange={() => update("appearance.themeMode", "claro")}
                />{" "}
                Claro
              </label>
              <label>
                <input
                  ...
                  name="theme"
                  value="escuro"
                  checked={themeMode === "escuro"}
                  onChange={() => update("appearance.themeMode", "escuro")}
                />{" "}
                Escuro
              </label>
              <label>
                <input
                  ...
                  name="theme"
                  value="sistema"
                  checked={themeMode === "sistema"}
                  onChange={() => update("appearance.themeMode", "sistema")}
                />{" "}
                Padrão do Sistema
              </label>
            </div>
          ...
          <div className="cfg-row">
            ...
            <div className="cfg-control">
              <select
                className="input"
                value={lang}
                onChange={(e) => update("language.locale", e.target.value)}
              >
        ...
        <section className="cfg-card" role="tabpanel">
          ...
          <div className="cfg-row">
            <div className="cfg-label">
              ...
              <small className="muted">
                Define quanto tempo de inatividade é necessário para seu status
                mudar de "Online" para "Ausente".
              </small>
            </div>
            <div className="cfg-control cfg-inline">
              <input
                ...
                value={autoMin}
                onChange={(e) =>
                  update("status.autoTimeoutMin", Number(e.target.value || 5))
                }
              />
