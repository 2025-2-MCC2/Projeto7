...
export default function PainelInicial() {
  ...
  const [perfil, setPerfil] = useState(() => load("perfil", { nome: "Usuário", fotoUrl: "", tipo: "mentor", ra: "12345" }));
  useEffect(() => save("perfil", perfil), [perfil]);
  useEffect(() => { document.title = "Lideranças Empáticas • Painel"; }, []);
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === "perfil") {
  ...
  const userId = useMemo(() => String(perfil?.ra || perfil?.nome || "anon"), [perfil?.ra, perfil?.nome]);
  const [creating, setCreating] = useState(false);
  const creatingTimerRef = useRef(null);
  const handleCreateGroupClick = useCallback((e) => {
  ...
  useEffect(() => { return () => clearTimeout(creatingTimerRef.current); }, []);
  ...
  const removerGrupo = useCallback(async (id) => { // Tornar async para API
  ...
  useEffect(() => {
      if (!loadingGrupos && gruposVisiveis.length > 0) {
          if (!grupoSelecionadoId || !gruposVisiveis.some(g => g.id === grupoSelecionadoId)) {
  ...
  const grupoParaDashboard = useMemo(() => {
      return grupoSelecionadoId ? gruposVisiveis.find((g) => g.id === Number(grupoSelecionadoId)) : null;
  ...
  const validarEvento = useCallback(() => {
    const errs = {};
    if (!formEvento.titulo || formEvento.titulo.trim().length < 3) errs.titulo = "Mínimo 3 caracteres.";
  ...
  const criarEvento = useCallback((e) => {
    e.preventDefault();
    if (!validarEvento()) return;
    const novo = {
      id: eventos.length ? Math.max(...eventos.map((ev) => ev.id)) + 1 : 1,
      ...formEvento, titulo: formEvento.titulo.trim(), descricao: (formEvento.descricao || "").trim(),
  ...
  const removerEvento = useCallback((id) => setEventos((prev) => prev.filter((e) => e.id !== id)), []);
  const nomeDoGrupo = useCallback((gid) => grupos.find((g) => g.id === gid)?.nome ?? "Sem grupo", [grupos]);
  ...
  const eventosPorDia = useMemo(() => {
    ...
    for (const [, arr] of map.entries()) { arr.sort((a, b) => (a.hora || "").localeCompare(b.hora || "")); }
    return map;
  }, [eventos, year, month]);
  const handlePrevMonth = useCallback(() => setCalDate((d) => new Date(d.getFullYear(), d.getMonth() - 1, 1)), []);
  const handleNextMonth = useCallback(() => setCalDate((d) => new Date(d.getFullYear(), d.getMonth() + 1, 1)), []);
  ...
  const validarAtividade = useCallback(() => {
    const errs = {};
    if (!formAtividade.titulo || formAtividade.titulo.trim().length < 3) errs.titulo = "Mínimo 3 caracteres.";
    setErrosAtividade(errs);
    return Object.keys(errs).length === 0;
  }, [formAtividade]);
  const criarAtividade = useCallback((e) => {
    e.preventDefault();
    if (!validarAtividade()) return;
    const nova = {
      id: atividades.length ? Math.max(...atividades.map((a) => a.id)) + 1 : 1,
      titulo: formAtividade.titulo.trim(), descricao: (formAtividade.descricao || "").trim(), concluida: false,
    };
           ...
           <main className="main-content">
               ...
               <header className="header">
                   ...
                   <div className="perfil" ref={profileRef}>
                        ...
                        >
                            {perfil.fotoUrl ? ( <img className="avatar" src={perfil.fotoUrl} alt="Foto do perfil" /> )
                        ...
                        {openProfileMenu && (
                          <div className="profile-menu">
                             <div className="profile-menu__header" id={profileMenuId} role="menu">
                               {perfil.fotoUrl ? (<img className="avatar small" src={perfil.fotoUrl} alt="Foto do perfil" />)
                               : (<span className="avatar small avatar-initials">{getInitials(perfil.nome)}</span>)}
                               <div className="profile-meta">
                                 <strong>{perfil.nome || "Usuário"}</strong>
                                 <small className="muted">online</small>
                               </div>
                             </div>
                             <button className="menu-item" onClick={() => { setOpenProfileMenu(false); navigate('/perfil'); }} role="menuitem">
                               <i className="fa-regular fa-user" aria-hidden="true"></i><span>Meu perfil</span>
                             </button>
                             <button className="menu-item" role="menuitem" onClick={() => { setOpenProfileMenu(false); navigate('/config'); }} title="Configurações" aria-label="Abrir configurações">
                               <i className="fa-solid fa-gear" aria-hidden="true"></i><span>Configurações</span>
                             </button>
                             <button className="menu-item danger" onClick={handleLogout}>
                               <i className="fa-solid fa-arrow-right-from-bracket" aria-hidden="true"></i><span>Sair</span>
                             </button>
                          </div>
                        )}
                   </div>
                   ...
                   <div className="grupos-header">
                       ...
                       {(perfil.tipo !== 'aluno' || gruposVisiveis.length === 0) && (
                         <button
                           ...
                           title="Criar novo grupo" aria-label="Criar novo grupo" disabled={creating}
                         >
                           {creating ? (<><i className="fa-solid fa-spinner fa-spin"></i><span>Criando…</span></>)
                           : (<><i className="fa-solid fa-plus"></i><span>Criar Grupo</span></>)}
                         </button>
                       )}
                   </div>
                   {loadingGrupos && <p>Carregando grupos...</p>}
                   {errorGrupos && <p className="message error" role="alert">{errorGrupos}</p>}
                   {!loadingGrupos && !errorGrupos && gruposVisiveis.length === 0 && <p>Nenhum grupo para exibir.</p>}

                   {!loadingGrupos && !errorGrupos && gruposVisiveis.map((g) => {
                       const percentFinanceiro = Math.min(((Number(g.progressoArrecadacao ?? 0) / Math.max(Number(g.metaArrecadacao ?? 1), 1)) * 100), 100);
                       ...
                       const percentAlimentos = metaNumAlimentos > 0 ? Math.min((progressoNumAlimentos / metaNumAlimentos) * 100, 100) : 0;

                       return (
                         <div key={g.id} className="grupo-card" role="button" tabIndex={0}
                              onClick={() => navigate(`/grupos/atividade/${g.id}`)}
                              onKeyDown={(e) => { if (e.key === 'Enter') navigate(`/grupos/atividade/${g.id}`); }}>
                           <div className="grupo-card__top"><h3>{g.nome}</h3></div>
                           {g.mentor && (
                             <div className="grupo-card__mentor" style={{ display: "flex", alignItems: "center", gap: 8, margin: "6px 0 4px" }}>
                               {g.mentorFotoUrl ? (<img className="avatar small" src={g.mentorFotoUrl} alt={`Mentor ${g.mentor}`} style={{ width: 28, height: 28, borderRadius: "50%" }} />)
                               : (<span className="avatar small avatar-initials" style={{ width: 28, height: 28, borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", background: "#d8f3dc", color: "#1b4332", fontWeight: 700 }}>{getInitials(g.mentor)}</span>)}
                               <span>Mentor: {g.mentor}</span>
                             </div>
                           )}
                           <p style={{marginBottom: '4px', fontSize: '0.9rem'}}>Meta Fin.: {currency(g.metaArrecadacao)} · Arrecadado: {currency(g.progressoArrecadacao)}</p>
                           <div className="progress-bar">
                               <div className="progress" style={{ width: `${percentFinanceiro}%` }} aria-valuenow={percentFinanceiro} aria-valuemin="0" aria-valuemax="100" role="progressbar"/>
                           </div>
                           {g.metaAlimentos && (
                             <div style={{marginTop: '8px'}}>
                               <p style={{marginBottom: '4px', fontSize: '0.9rem'}}>Meta Alim.: {g.metaAlimentos} · Progresso: {progressoNumAlimentos} {metaNumAlimentos > 0 ? `/ ${metaNumAlimentos}` : ''}</p>
                               <div className="progress-bar">
                                   <div className="progress" style={{ width: `${percentAlimentos}%` }} aria-valuenow={percentAlimentos} aria-valuemin="0" aria-valuemax="100" role="progressbar"/>
                               </div>
                             </div>
                           )}
                           <div className="grupo-card__actions" style={{ display: "flex", gap: 8, marginTop: '12px' }} onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                               <button className={`btn btn-secondary btn--edit ${editingId === g.id ? "is-loading" : ""}`} onClick={(e) => { e.stopPropagation(); handleEditClick(g.id); }} title="Editar grupo" aria-label={`Editar grupo ${g.nome}`} disabled={editingId === g.id}>
                                  {editingId === g.id ? (<><i className="fa-solid fa-spinner fa-spin"></i><span>Editando…</span></>) : (<><i className="fa-solid fa-pen-to-square"></i><span>Editar</span></>)}
                               </button>
                               <button className={`btn btn-danger btn--delete ${confirmDeleteId === g.id ? "confirm" : ""}`} onClick={(e) => { e.stopPropagation(); handleDeleteClick(g.id); }} title={confirmDeleteId === g.id ? "Confirmar exclusão" : "Excluir grupo"} aria-label={ confirmDeleteId === g.id ? `Confirmar exclusão do grupo ${g.nome}` : `Excluir grupo ${g.nome}` }>
                                   {confirmDeleteId === g.id ? (<><i className="fa-solid fa-check"></i><span>Confirmar</span></>) : (<><i className="fa-solid fa-trash"></i><span>Excluir</span></>)}
                               </button>
                           </div>
                         </div>
                       );
                 ...
                 <div className="dashboard-container">
                    <div className="dashboard-header">
                        ...
                        <select id="grupo-select" className="input" value={grupoSelecionadoId ?? ''} onChange={(e) => setGrupoSelecionadoId(e.target.value)}>
                          ...
                          {!loadingGrupos && gruposVisiveis.length === 0 && <option>Nenhum grupo</option>}
                          {!loadingGrupos && gruposVisiveis.map((g) => (<option key={g.id} value={g.id}>{g.nome}</option>))}
                        </select>
                    </div>
                    {grupoParaDashboard ? (<Dashboard grupo={grupoParaDashboard} />)
                     : (<p>{loadingGrupos ? 'Carregando...' : (perfil.tipo === 'aluno' ? 'Você ainda não faz parte de um grupo.' : 'Selecione um grupo para ver o dashboard.')}</p>)}
                 </div>
               )}
                 ...
                 <section className="grupos-section agenda-section">
                    ...
                    <div className="filters">
                        ...
                        <div className="filters__date">
                          {filtroData ? (<><span className="chip">Data: {new Date(filtroData + "T00:00:00Z").toLocaleDateString("pt-BR", {timeZone: 'UTC'})}</span><button className="btn btn-ghost" onClick={() => setFiltroData("")}>Limpar data</button></>)
                        ...
                        <div className="calendar-grid">
                            ...
                            {Array.from({ length: daysInMonth }).map((_, i) => {
                                ...
                                return ( <button key={ymd} className={`day ${isToday ? "today" : ""} ${isSelected ? "selected" : ""} ${dayEvents.length ? "has-events" : ""}`} onClick={() => handleSelectDay(dayNum)}>
                                    ...
                                    <div className="events-in-day">{dayEvents.slice(0, 2).map((ev) => (<span key={ev.id} className="event-chip" title={`${ev.hora ? ev.hora + " · " : ""}${ev.titulo}`}>{ev.hora ? `${ev.hora} ` : ""}{ev.titulo}</span>))}{dayEvents.length > 2 && <span className="more-chip">+{dayEvents.length - 2}</span>}</div>
                                  </button> );
                            })}
                        </div>
                    </div>
                    {eventosOrdenadosFiltrados.length === 0 ? (<p>Nenhum evento encontrado.</p>)
                     : (<div className="event-list">{eventosOrdenadosFiltrados.map((ev) => (
                            <div key={ev.id} className="event-card">
                               <div className="event-card__left"><div className="event-date"><span className="day">{new Date(ev.data + "T00:00:00Z").getUTCDate()}</span><span className="month">{new Date(ev.data + "T00:00:00Z").toLocaleDateString("pt-BR", { month: "short", timeZone: 'UTC' })}</span></div></div>
                               <div className="event-card__content"><h3>{ev.titulo}</h3><p className="muted">{ev.hora} · {nomeDoGrupo(ev.grupoId)}</p>{ev.descricao && <p>{ev.descricao}</p>}</div>
