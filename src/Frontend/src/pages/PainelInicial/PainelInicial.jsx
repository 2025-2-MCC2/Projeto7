...
export default function PainelInicial() {
  ...
  useEffect(() => { document.title = "Lideranças Embaticas • Painel"; }, []);

  // 🔄 Sincroniza o perfil se ele for alterado em outra aba/guia (PerfilPage)
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === 'perfil') {
  ...
  // --- Eventos/Agenda/Atividades (inalterados) ---
            ...
            {gruposVisiveis.map((g) => (
              <div key={g.id} className="grupo-card">
                ...
                <div className="progress-bar">
                  <div
                    className="progress"
                    style={{ width: `${Math.min(((g.progressoArrecadacao || 0) / (g.metaArrecadacao || 1)) * 100, 100)}%` }}
                  />
                </div>
